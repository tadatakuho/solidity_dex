...
contract Dex {
  ...
  mapping(address => bool) public supportedTokenAddrs;

  modifier supportsToken(address _tokenAddr) {
    require(supportedTokenAddrs[_tokenAddr] == true, "This token is not supported");
    _;
  }

  constructor(address[] memory _tokenAddr) {
    for(uint i=0; i< _tokenAddr.length; i++) {
      supportedTokenAddrs[_tokenAddr[i]] = true;
    }
  }
  ...
  function sellToken(address _tokenAddr, uint256 _cost, uint256 _amount) external supportsToken(_tokenAddr) {
    ERC20 token = ERC20(_tokenAddr);
    require(token.balanceOf(msg.sender) >= _amount, "Insufiicient token balance");
